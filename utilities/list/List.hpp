...
public:
    ...
    ListNodePosi(T) first() const { return _header->succ; } // 返回首节点
    ListNodePosi(T) last() const { return _trailer->pred; } // 返回尾节点
...
template <typename T> int List<T>::clear()  //清空,返回旧表长
{
    int oldSize = _size;
    while(_size) remove(_header->succ);
    return oldSize;
}
...
template <typename T> int List<T>::deduplicate()  //乱序的长耗时唯一化,通过find遍历函数知道唯一性
{
    if(_size < 2) return 0;
    int oldSize = _size;
    ListNodePosi(T) p = _header; Rank r = 0;
...
template<typename T> int List<T>::uniquify()
{
    ...
    for(p  = _header,q = p->succ; q != _trailer;p = q, q = q->succ)  //从左到右遍历,删除重复元素
...
{
    for(ListNodePosi(T) p = _header->succ; p != _trailer; p = p->succ)
        visit(p->data);
}
...
//查找
template <typename T> ListNodePosi(T) List<T> ::search(const T& e , int n,ListNodePosi(T) p) const //从位置p开始查找n个元素const
{
    while(0<=n--) 
...
//归并排序
template <typename T> void List<T>::merge(ListNodePosi(T) &p, int n,List<T> &L,ListNodePosi(T) q, int m)  //归并排序
{
